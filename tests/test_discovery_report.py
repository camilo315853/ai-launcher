...
class TestGenerateDiscoveryReport:
    ...
    def test_report_contains_header(self, sample_projects, sample_providers, tmp_path):
        report = generate_discovery_report(sample_projects, sample_providers, [tmp_path])
    ...
    def test_report_shows_manual_marker(self, tmp_path, sample_providers):
        ...
        report = generate_discovery_report([manual_project], sample_providers, [tmp_path])
    ...
    def test_report_truncates_at_10_projects(self, tmp_path, sample_providers):
        ...
        for i in range(15):
            ...
            projects.append(
                Project(
                    path=p, name=f"proj-{i}", parent_path=tmp_path,
                    is_git_repo=True, is_manual=False,
                )
            )
        report = generate_discovery_report(projects, sample_providers, [tmp_path])
        assert "5 more projects" in report

    def test_report_shows_installed_provider(self, sample_projects, sample_providers, tmp_path):
        report = generate_discovery_report(sample_projects, sample_providers, [tmp_path])
        assert "✓ Claude Code" in report
        assert "v2.1.37" in report
        assert "Ready to use" in report

    def test_report_shows_not_installed_provider(self, sample_projects, sample_providers, tmp_path):
        report = generate_discovery_report(sample_projects, sample_providers, [tmp_path])
        assert "✗ Gemini CLI" in report
        assert "Not installed" in report
        assert "https://ai.google.dev/gemini-api/docs/cli" in report

    def test_report_shows_context_stats(self, sample_projects, sample_providers, tmp_path):
        report = generate_discovery_report(sample_projects, sample_providers, [tmp_path])
    ...
    def test_report_tilde_paths(self, sample_projects, sample_providers):
        ...
        report = generate_discovery_report(sample_projects, sample_providers, [scan_path])
...
class TestShowDiscoveryReport:
    ...
    def test_show_prints_report(self, sample_projects, sample_providers, tmp_path, capsys):
